...
def test_rtree_create_entry_read_lin(dimensions: int, count: int, low: int, high: int):
    try:
        os.remove(TESTING_DIRECTORY + DATABASE_FILE_TEST)
    except FileNotFoundError:
        pass
    ...
    for c, position in enumerate(created_entries_positions):
        ...
        found_lin_entry = database.linear_search_entry(found_entry.coordinates)

        assert found_lin_entry is not None
        assert found_lin_entry.data["c"] == c
        assert found_lin_entry.data["coordinates"] == found_entry.coordinates
    # for c, position in enumerate(created_entries_positions):
    #     database.l
