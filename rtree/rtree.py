...
class RTree:
    ...
    def insert_entry(self, new_entry: DatabaseEntry):
        ...
        if desired_node.is_full():
            ...
            root_node = self.__get_node(self.root_id)
            if not root_node.mbb.contains_inner(new_entry.get_mbb()):
                print("root must propagate", root_node.mbb, "to contain", new_entry.get_mbb())
                self.propagate_mbb_increase(desired_node)
